...
$botonCalcularTiempo.onclick = function(){
    ...
    for (let i = 1; i <= numerodeClases; i++){
        ...
        totalDeMinutos += Number(document.getElementById(`minutos-clase${i}`).value);
        totalDeSegundos += Number(document.getElementById(`segundos-clase${i}`).value);
    }
    $resultadoFinal.innerText = imprimirResultado (totalDeHoras, totalDeMinutos, totalDeSegundos);
}
...
function imprimirResultado(horas, minutos, segundos){
    ...
    if (minutosTotales > segundosYMinutos){
        ...
        horasTotales += ((minutosTotales - (minutosTotales % segundosYMinutos)) / segundosYMinutos)
    }
    horasTotales += horas;
